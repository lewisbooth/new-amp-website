...
!(function(A, e, o) {
  ...
  function a() {
    ...
    for (var r in s)
      if (s.hasOwnProperty(r)) {
        ...
        for (t = n(e.fn, "function") ? e.fn() : e.fn, a = 0; a < A.length; a++)
          (l = A[a]),
            (i = l.split(".")),
            1 === i.length
              ? (Modernizr[i[0]] = t)
  ...
  function i(A, e) {
    if ("object" == typeof A) for (var o in A) d(A, o) && i(o, A[o]);
      ...
      var n = A.split("."),
        a = Modernizr[n[0]];
      if ((2 == n.length && (a = a[n[1]]), "undefined" != typeof a))
        return Modernizr;
...
var images = document.querySelectorAll(".portfolio-images-image");
var playButtons = document.querySelectorAll(".play");
var pauseButtons = document.querySelectorAll(".pause");

Modernizr.on("videoautoplay", function(result) {
  if (!result) {
    for (var i = 0; i < playButtons.length; i++) {
      playButtons[i].classList.remove("off");
    }
    for (var i = 0; i < pauseButtons.length; i++) {
      pauseButtons[i].classList.add("off");
    }
  }
});

for (var i = 0; i < images.length; i++) {
  if (images[i].innerHTML.indexOf("video") != -1) {
    var video = images[i].querySelector("video");
    var play = images[i].querySelector(".play");
    var pause = images[i].querySelector(".pause");

    pause.addEventListener("click", e => {
      var pause = e.target;
      var play = e.target.parentNode.childNodes[1];
      var video = e.target.parentNode.childNodes[0];
    ...
    play.addEventListener("click", e => {
      var play = e.target;
      var pause = e.target.parentNode.childNodes[3];
      var video = e.target.parentNode.childNodes[0];
